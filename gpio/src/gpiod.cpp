...
static const char *const shortopts = "+hvln:srfdF:";

static void print_help(void)
{
    printf("Usage: gpiod [OPTIONS] <chip name/number> <offset 1> <offset 2> ...\n");
...
static const char *trigger_script = "/data/gpio/triggers.sh";
                       ...
                       int event_type)
{
    ...
    ALOGI("event: %s offset: %u timestamp: [%8ld.%09ld]\n",
           evname.c_str(), offset, ts->tv_sec, ts->tv_nsec);
             ...
             const struct timespec *timeout, void *data)
{
    ...
    struct mon_ctx *ctx = (mon_ctx *)data;
    int cnt, ts, ret;
    unsigned int i;
             ...
             const struct timespec *timestamp)
{
    ...
    // /data/gpio/triggers.sh gpio 0|1 timestamp
    // line_offset
    // event_type == GPIOD_CTXLESS_EVENT_CB_RISING_EDGE ? 0 : 1
    // timestamp
    char offset_str[3];
    sprintf(offset_str, "%d", line_offset);
    char timestamp_str[256];
    sprintf(timestamp_str, "%8ld.%09ld", timestamp->tv_sec, timestamp->tv_nsec);
    char edge_str[2];
    sprintf(edge_str, "%d", event_type == GPIOD_CTXLESS_EVENT_CB_RISING_EDGE ? 0 : 1);
    char system_str[1024];
    sprintf(system_str, "/system/bin/sh %s %s %s %s", trigger_script, offset_str, edge_str, timestamp_str);

    if (ctx->debug) ALOGI("%s", system_str);
    system(system_str);
    ctx->events_done++;
}

static bool trigger_script_exists() {
    struct stat buf;
    return stat(trigger_script, &buf) == 0;
}
...
int main(int argc, char **argv)
{
    ...
    for (i = argvIndex + 1; i < argcList; i++) {
        offset = strtoul(argvList[i], &end, 10);
        if (*end != '\0' || offset > INT_MAX) {
            die_logd("invalid GPIO offset: %s", argvList[i]);
        }
        offsets[num_lines] = offset;
        num_lines++;
    }

    if (!trigger_script_exists()) {
       die_logd("trigger script not found"); 
    }
    ctx.sigfd = make_signalfd();

    ret = gpiod_ctxless_event_monitor_multiple(argvList[argvIndex], event_type,
                           offsets, num_lines,
                           active_low, "gpiod",
