			 ...
			 const struct timespec *timestamp)
{
	...
	//char *args[]={"/system/bin/sh", "/data/gpio/triggers.sh", offset_str, edge_str, timestamp_str, NULL}; 
	//execvp(args[0],args); 
	char system_str[1024];
	sprintf(system_str, "/system/bin/sh /data/gpio/triggers.sh %s %s %s", offset_str, edge_str, timestamp_str);
	//ALOGI("%s", system_str);
    system(system_str);
	ctx->events_done++;
}
...
static bool trigger_script_exists() {
    struct stat buf;
    return stat("/data/gpio/triggers.sh", &buf) == 0;
}
